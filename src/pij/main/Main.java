...
public class Main {
    /**
     * Test Feature:
     * A humanPlayer can be substituted with another ComputerPlayer, by changing all the related parameters
     * and the computer players can play among themselves till the end
     */
    public static void main(String[] args) {
        try {
            TileBag tileBag = TileBag.getTileBag(); // one game needs only one tileBag
            GameBoard gameBoard = GameBoard.getGameBoard(); // one game needs only one game board
            ...
            while (!gr.isGameOver()) {
                ...
                while (!isMoveVerified) {
                    ...
                    if (playersMove.equals(PASS)) {
                        ...
                        MoveValidator mv = new MoveValidator(playersMove, gameBoard);
                        mv.createMove(); // new unverified move is created
